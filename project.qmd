...
```{r}
# Import Packages
pacman::p_load(sf, tmap, tidyverse, sfdep, readxl, Kendall, plotly, plyr, SpatialAcc, ggstatsplot, reshape2, fca, sp, caret, stplanr, broom, geojsonio, jsonlite)
```
...
```{r}
mrt_tour_Hansen <- st_join(mrt_tour_Hansen, mpsz, 
                          join = st_intersects)
```

```{r}
ggplot(data=mrt_tour_Hansen, 
       aes(y = log(accHansen), 
           x= REGION_N)) +
  geom_boxplot() +
