...
tm_shape(mpsz[mpsz$REGION_N == 'CENTRAL REGION',]) +
  tm_polygons() +
tm_shape(mpsz_mrt[mpsz_mrt$REGION_N == 'CENTRAL REGION',]) +
...
```{r}
hex_mpsz <- st_join(hex_tour_Hansen, mpsz)
```

```{r}
hex_mpsz_region <- hex_mpsz %>%
  filter(REGION_N == "CENTRAL REGION")
```

```{r}
summary(hex_mpsz_region)
```

```{r}
tmap_mode("view")
tm_shape(hex_mpsz[hex_mpsz$REGION_N == 'CENTRAL REGION',],
...
tm_shape(mpsz_mrt[mpsz_mrt$REGION_N == 'CENTRAL REGION',]) +
  tm_dots(alph=0.5, size=0.07)+
  tm_view(set.zoom.limits = c(11,14)) +
tm_shape(tourism_mpsz[tourism_mpsz$REGION_N == 'CENTRAL REGION',]) +
  tm_symbols(size = 0.1) +
...
```{r}
tourism_mpsz <- st_join(tourism, mpsz)
```
...
tm_shape(tourism_mpsz[tourism_mpsz$REGION_N == 'CENTRAL REGION',]) +
  tm_symbols(size = 0.5) +
tm_shape(mrt_mpsz[mrt_mpsz$REGION_N == 'CENTRAL REGION',]) +
  tm_dots(alph=0.1, size=0.1)+
  tm_view(set.zoom.limits = c(11,14)) +
tm_shape(mrt_tour_Hansen[mrt_tour_Hansen$REGION_N == 'CENTRAL REGION',]) +
